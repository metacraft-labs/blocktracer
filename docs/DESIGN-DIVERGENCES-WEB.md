...
| **D-11** | Viewport-relative caps | `44vh` — `--bt-layout-tabmenu-max-height`, the ceiling the file MENU hits, and scrolls past | **No token.** Neither lineage expresses a length as a share of the viewport; the brand’s ramps are px and the product lineage’s are px. | This is the one bound in the web lineage whose subject is the READER’S SCREEN rather than the layout, which is why it is not another number in D-07. A tab strip is a way in to the document beneath it, and a 32-file bundle wrapped to seven rows at 1280 and took a quarter of the pane — so the overflow has to be bounded, and bounded against the thing that made it wrong. A px ceiling cannot do that: any value large enough to be useful on a desktop is most of a phone. Does a design system own proportional bounds at all, or is “a share of the viewport” inherently a per-component layout decision the system should decline to name? **What changed, and why the row survives it — twice.** First, the strip stopped wrapping at rest: it is one row that scrolls sideways, so the cap stopped bounding the resting strip and bounded the state a reader opens. Second — this round — the state a reader opens stopped being IN FLOW. The disclosure unfolded the list in place and measured EIGHT rows and 196px at 1280, *more* than the 172px wrap it was introduced to fix; the reporter asked for “a drop down menu for the tabs that don’t fit”, and a menu is absolutely positioned, drawn over the source, and displaces nothing. So the token was RENAMED rather than re-pointed — `tabstrip` to `tabmenu` — because the quantity it bounds is no longer the strip’s share of the document, and a rename is what forces every reference to be re-read instead of silently inheriting the old meaning. It was also RAISED, 22vh to 44vh, for the same reason: a cap on an in-flow list is protecting the document’s remaining height and small is the point, whereas a cap on an overlay is protecting only against running off the screen — and 22vh showed six files of a thirty-two-file bundle, which is a menu no better than the row it was opened from. MEASURED at 44vh: 352px at 800px tall, 396px at 900 and 475px at 1080, showing 15 to 20 of the 32 and scrolling for the rest, and inside the Code pane's own box at 1280, 1440 and 1920 — the journey asserts that containment rather than the number, because a cap that has outgrown its pane is the one failure a viewport-relative length can still reach. The argument above is untouched by either move: the quantity being bounded is still the viewport and the failure it prevents is still proportional to it. Note the cap is on the container and not on the item count, so it never removes a file — the menu scrolls and every file stays a reachable link. Asserted by the journey `a-long-file-list-fits-on-one-row`. |
...
| `--bt-layout-tabmenu-max-height` | bkLiteral | `44vh` | [D-11](#d-11) |
