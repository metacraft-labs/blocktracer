...
export const EXPECTATIONS = [
  ...
  },
  {
    ...
    mustShow: [
      ...
      "THE CALL TRACE NOW LISTS ITS FRAMES ON THE SERVED PAGE, and this expectation inverted — it used to say the served capture shows the sentence and not the rows. The frames are lifted out of the container by `tools/chain/derive-calltrace.mjs` the same way the instruction listing is (CHAIN-CAPTURE.md §6.6), so the pane must show ROWS: `<toplevel>` and `enqueued-call-0`, nested, with the frame the session is stopped in marked. They carry NO file and NO line — the recorder placed them on its pseudo-path — so a filename or a `:line` appearing on a row would be invented source and is a P1. Grade the rows as content: does a two-row call trace read as a real stack, or as a stub?",
