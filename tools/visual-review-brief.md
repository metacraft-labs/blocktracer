...
- THE CALL TRACE IS A NOIR CALL TREE, AND IT IS THE BIGGEST THING ON THIS PAGE. The frames are lifted out of the container by `tools/chain/derive-calltrace.mjs` (CHAIN-CAPTURE.md §6.6). This expectation has now inverted TWICE and the second inversion is the one to hold: it once said the served capture shows a sentence and not rows; it then said the rows are `<toplevel>` and `enqueued-call-0` and nothing else. The container for this transaction carries FORTY-SIX frames, nested six deep — `<toplevel>` → `enqueued-call-0` → `FeeJuice::public_dispatch`, and under that the real Noir call structure of a fee-juice public call: `derive_deserialize`, `_increase_public_balance`, `Map::at`, `PublicMutable::read` and `::write`, `poseidon2_hash_with_separator`. A pane showing two rows is now a REGRESSION, not the expected state. Grade the tree as content: at this depth, is the nesting readable, can the eye follow a subtree, and does the pane stay legible at six levels of indent?
