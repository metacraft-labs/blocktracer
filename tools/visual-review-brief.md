...
*79 named views, 79 blocks — generated from `tools/capture/expectations.mjs`. 46 are currently `ready` to capture; the rest are listed with the reason their route or state is not served yet, because a view that cannot be captured still has to have an expectation before it can be.*
