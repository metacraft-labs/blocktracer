...
*85 named views, 85 blocks — generated from `tools/capture/expectations.mjs`. 51 are currently `ready` to capture; the rest are listed with the reason their route or state is not served yet, because a view that cannot be captured still has to have an expectation before it can be.*
...
- *Inherited backbone `site-chrome` (Page-Descriptions §2, §12, Design-System §2):*
  - The site footer, closing the page: the product line, the About / Chains / Keyboard shortcuts links, the Metacraft Labs / CodeTracer credit and the source link. Each link is labelled with the NAME OF THE PAGE it lands on rather than the name of its route — `/settings` is titled `Keyboard shortcuts` and the link says so; a footer reading `Settings` promises a preferences surface in general and delivers one setting. It ENDS the page — on a page shorter than the viewport it sits at the bottom of the viewport, not partway down it with canvas underneath. It carries NO disclosure sentence of any kind: no claim about whose data the page shows, and no privacy claim either. Provenance is the per-page marker's job, and the privacy claims are answered on `/about`, linked from the footer's own row. A footer sentence asserting provenance, directing the reader elsewhere to find it, or reassuring them that no account is required, is a finding — the last of those reads as an answer to a question no visitor asked.
...
- AN INSTRUCTION-LEVEL SESSION, AND ITS LISTING. This is the part that makes the view worth capturing and it is NOT what the synthetic session shows. Nothing resolved this contract's compiled artifact, so no step resolves to SOURCE TEXT the Code pane could print — and the recording still carries a coordinate for every step. (This is about source TEXT and about the CODE pane only. The call trace's frames do carry a Noir file and line; having a coordinate is not the same as having the file it points into, and the missing half is the text.) The Code pane must show BOTH: the reason there is no source, and an INSTRUCTION LISTING under it — one row per recorded step, each opening with the program counter the VM was at, with the step the session is stopped at marked exactly as a current source line would be. Grade the listing as content: are the columns legible, is the marked row findable, does the pane read as a debugger stopped somewhere rather than as a wall of hex?
- THE CALL TRACE IS A NOIR CALL TREE, AND IT IS THE BIGGEST THING ON THIS PAGE. The frames are lifted out of the container by `tools/chain/derive-calltrace.mjs` (CHAIN-CAPTURE.md §6.6). This expectation has now inverted TWICE and the second inversion is the one to hold: it once said the served capture shows a sentence and not rows; it then said the rows are `<toplevel>` and `enqueued-call-0` and nothing else. The container for this transaction (`0x20ed5b91…`) carries FORTY-SIX frames over THIRTY-THREE distinct Noir functions, nested NINE deep beneath the enqueued call — `<toplevel>` → `enqueued-call-0` → `FeeJuice::public_dispatch`, and under that the real Noir call structure of a fee-juice public call: `derive_deserialize`, `_increase_public_balance`, `Map::at`, `PublicMutable::read` and `::write`, `poseidon2_hash_with_separator`. A pane showing two rows is now a REGRESSION, not the expected state — two rows is what the other twenty-four captured transactions still carry, and this is the one that does not. Grade the tree as content: at nine levels of indent, is the nesting readable, can the eye follow a subtree back to its parent, and does the deepest row still have usable width for its name?
- TWO FOLDED GROUPS, DRAWN AS REAL DISCLOSURES THE READER CAN OPEN. Both are `Poseidon2::hash`, folded by the `vendored-crate` rule (`tools/chain/fold_rules.mjs`) because poseidon2 is a third-party crate vendored through nargo and the single largest consumer of steps in a typical Aztec public call. Each is a `<details>` whose `<summary>` is an ordinary row carrying a count of what is behind it — `2 frames · 22 steps` on the first, `2 frames · 6 steps` on the second. FOLDED IS NOT ELIDED: every frame the recorder wrote is present in the markup at its real depth, so the reader opens the triangle rather than being told a number and refused the rows. Grade the affordance: is a folded frame legible as a row that opens, is the count a useful reason to open it, and does the closed state read as the pane's own judgement about library code rather than as missing data?
- THE VALUES PANE STILL DECLINES, IN ITS OWN WORDS. Naming a local needs the debug symbols from the contract's compiled artifact, and none resolved. It may not sit blank.
...
- INVENTED SOURCE **IN THE CODE PANE**. No compiled artifact resolved for this contract, so the CODE pane has no source text to draw and may not fabricate any: no SOURCE LINE of any language, no lexer's colours over its rows, no `.srctabs` file strip, no file tab. That is still the most serious failure this page could contain. NOTE HOW NARROW THIS RULE HAS BECOME, because it has been redrawn twice and a reviewer applying the old width will file a P1 against correct behaviour. Rows of PROGRAM COUNTERS in the Code pane are not invented source — they are coordinates the recording carries, and the pane is required to show them. NEITHER ARE THE CALL TRACE'S FUNCTION NAMES AND LINE NUMBERS: the container records a Noir `path_id` and line for each frame, so `FeeJuice::public_dispatch` and a `:203` beside it are RECORDED FACTS and are required (see mustShow). The rule that remains is about SOURCE TEXT, and it is scoped to the Code pane.
- A FILE PATH PAINTED ON A CALL TRACE ROW. The frames carry full absolute paths (`/home/aztec-dev/aztec-packages/...`), and the row deliberately does not paint them — the name takes the width and the path rides in the row's tooltip and `data-module`. This is a deliberate treatment, not an omission: a visible path here would cut both the name and the path to half-identifiers on a narrow pane, which is the defect it was changed to fix. A path rendered into the row is a P2.
...
- A blank pane with no explanation. Empty is correct in the Values pane; unexplained is not. An empty CODE pane is a defect rather than a state — the listing is what fills it — and an empty CALL TRACE is now one too, for the same reason and by the same mechanism: the frames are derived and committed, so a pane showing the old `They are listed once the session is live.` sentence means the derivation did not reach the page.
...
- The Code pane is no longer the emptiest pane, and it carries three things stacked above its rows: the position sentence, the reason there is no source, and the listing's column caption. Judge that stack. Three strips before the first row is a lot of chrome; say whether a reader reaches the listing quickly and whether any of the three could be doing less. Note that it is no longer the FULLEST pane either — the 46-frame call trace is — so also say which pane the eye goes to first, and whether that is the right one for a reader arriving at a real testnet session.
...
- The `About this data` section, and it is SHORT: two sentences saying the data is real, that it is a preliminary export, and what timespan it covers. Anything longer is the finding — this section was cut back on 2026-09-04 from four generated paragraphs a reader was told narrated the capture rather than the data. The capture date, the published block window, the counts of what was watched against what was recorded, the retention boundary and the coverage mode are all deliberately NOT here; they describe the deployment's wiring or restate what the tables above already say, and their absence is not a finding. This view is partly a CONTROL — the surrounding page must not have changed because the data is real.
...
### View: `chain-overview--testnet-frames`
