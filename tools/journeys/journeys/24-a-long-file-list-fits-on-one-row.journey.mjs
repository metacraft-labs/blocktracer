...
// announced rather than silent) and the note on `--bt-layout-tabstrip-max-height`
...
//
// WHY THE DISCLOSURE IS ASSERTED TOO
// ----------------------------------
// One row that scrolls sideways keeps every file reachable in the sense that a
// mouse can get to it, and that is a thin sense: at 1280 the Code pane is 604px
// and the bundle is 3623px of tabs, so the file a visitor wants is on average a
// two-screen drag away. The control that opens the whole list is what makes the
...
export const claim =
  ...
  "with the file being read always named and every other file still reachable.";
export const spec =
  "Page-Descriptions.md §13; docs/DESIGN-DIVERGENCES-WEB.md D-11 — BlockTracer";
// 9, and every one of them runs exactly once: the per-subject work accumulates
...
const readStrip = (page) =>
  page.evaluate(() => {
    ...
    // THE DISCLOSURE, driven rather than inspected: a control that is present
    // and does nothing is the thing worth catching.
    ...
    if (opener) {
      ...
      const openHeight = Math.round(strip.getBoundingClientRect().height);
