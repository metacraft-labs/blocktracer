...
suite "M8b — the crawl surface is unchanged":
  ...
  test "the debug route is rendered but NOT submitted to search engines":
    ...
    check "/search" in all
    # `/settings` is in NEITHER list now — the page was removed, so it is not
    # an excluded route, it is not a route. Asserted over `all` rather than
    # over `submitted`, because "absent from the sitemap" is also true of a
    # route that merely stopped being generated by mistake.
    check "/settings" notin all
    # N2 still HAS a member, so the exclusion above is not vacuous.
