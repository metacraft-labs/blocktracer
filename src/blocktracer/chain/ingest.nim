...
proc curationWindow*(blockHeights: seq[int];
  ...
  # Nothing recorded: the newest transaction-free run, capped.
  var hi = highest
  while hi >= lowest and isTraceless.getOrDefault(hi, false): dec hi
  if hi < lowest: return
  var lo = hi
  var span = 1
  while lo - 1 >= lowest and span < SurveyBlocks and
        not isTraceless.getOrDefault(lo - 1, false):
    dec lo
    inc span
  result = CurationWindow(lo: lo, hi: hi, found: true,
    why: "Nothing on this chain has been recorded yet, so there is no span of " &
         "recordings to publish. What is published is the newest run of blocks " &
         "that settled no transaction at all — blocks " & $lo & "–" & $hi &
